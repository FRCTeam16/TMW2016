...
Arm::Arm() : Subsystem("Arm") {
    ...
    highShooter = Preferences::GetInstance()->GetFloat("LongShooterArm", 720);
    lowShooter = Preferences::GetInstance()->GetFloat("ShortShooterArm", 570);
    cout << "Arm->HighShooter = " << highShooter << " | ShortShooter = " << lowShooter << '\n';

    dartLeftMaxForward = 5;
    dartLeftMaxReverse = -12;
    dartRightMaxForward = 5;
    dartRightMaxReverse = -12;
	dartMaxForwardDefault = 5;
	dartMaxReverseDefault = -12;
	dartOutputLimit = 1.0;
	dartCoolCount = 0;
	dartRightHotCount = 0;
	dartLeftHotCount = 0;
...
void Arm::DartPosition(int pos) {
	...
	dartRight->SetControlMode(CANTalon::kPosition);
	dartLeft->Set(pos);
	float wallshot = wallshotDartEnabled ? Preferences::GetInstance()->GetFloat("WallShotOffset", 0) : 0;
	dartRight->Set(pos + dartOffset + wallshot);
}
...
void Arm::SetWallshotDart(const bool enabled) {
	wallshotDartEnabled = enabled;
}
bool Arm::GetWallshotDart() const {
	return wallshotDartEnabled;
}
...
void Arm::DartManager() {
	...
	if(dartCoolCount > coolCountLimit) {
		dartOutputLimit = 1.0;
	}

	if(dartOutputUnlimited || wallshotDartEnabled) {
		dartLeftMaxForward = 12;
		dartLeftMaxReverse = -12;
		dartRightMaxForward = 12;
		dartRightMaxReverse = -12;
	}
	else {
		dartLeftMaxForward = dartMaxForwardDefault;
		dartLeftMaxReverse = dartMaxReverseDefault;
		dartRightMaxForward = dartMaxForwardDefault;
		dartRightMaxReverse = dartMaxReverseDefault;
		if(GetCorrectedDartDifference() > 5) { //right dart is lower than left
			if(dartLeft->Get() > 0) { //going down
				dartRightMaxForward = 2;
			}
			if(dartLeft->Get() < 0) { //going up
				dartLeftMaxReverse = -1;
			}
		}

		else if(GetCorrectedDartDifference() < -5) { //left dart lower than right
			if(dartLeft->Get() > 0) { //going down
				dartLeftMaxForward = 2;
			}
			if(dartLeft->Get() < 0) { //going up
				dartRightMaxReverse = -1;
			}
		}
	}
	dartLeft->ConfigPeakOutputVoltage(dartLeftMaxForward,dartLeftMaxReverse);
...
void Arm::SMDB() {
	...
	SmartDashboard::PutBoolean("Wallshot Dart", wallshotDartEnabled);
	...
	SmartDashboard::PutNumber("DartRight Pos", dartRight->GetPosition());
}
